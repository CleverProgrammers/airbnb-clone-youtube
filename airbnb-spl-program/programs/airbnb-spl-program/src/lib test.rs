...
declare_id!("5bzDL5NHSnQbrcTfFtZvBWrC1cLTy4agJKuiUNcEw8yk");
...
pub mod clever_airbnb {
    ...
    pub fn book_airbnb(ctx: Context<BookAirbnb>,date: String) -> Result<()> {
        let booking_account = &mut ctx.accounts.booking_account;
        
        // Fill contents with argument
        booking_account.authority = ctx.accounts.booking_authority.key();
        booking_account.airbnb = ctx.accounts.airbnb_account.key();
        booking_account.user = ctx.accounts.booking_authority.key();
        booking_account.date = ctx.accounts.booking_authority.key();
